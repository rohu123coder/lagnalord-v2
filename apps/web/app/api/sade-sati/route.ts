...
export async function POST(req: Request) {
  ...
  try {
    const backendRes = await fetch(`${BACKEND_URL}/api/sade-sati/calculate`, {
      method: "POST",
    ...
    if (!backendRes.ok) {
      ...
      console.error(
        "Backend sade-sati error:",
        backendRes.status,
        errText.slice(0, 500)
      );
      return NextResponse.json(
        { error: "Sade Sati calculation service unavailable. Please try again." },
        { status: 503 }
      );
    }
      ...
      return NextResponse.json(
        {
          error:
            backendJson.error ??
            "Sade Sati calculation failed. Please verify birth details.",
        },
        { status: 502 }
      );
    }
    ...
    console.error("Sade Sati proxy error:", e);
