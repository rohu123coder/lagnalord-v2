...
export async function POST(req: Request) {
  ...
  try {
    const backendRes = await fetch(`${BACKEND_URL}/api/lal-kitab/calculate`, {
      method: "POST",
    ...
    if (!backendRes.ok) {
      ...
      console.error(
        "Backend lal-kitab error:",
        backendRes.status,
        errText.slice(0, 500)
      );
      return NextResponse.json(
        { error: "Lal Kitab calculation service unavailable. Please try again." },
        { status: 503 }
      );
    }
      ...
      return NextResponse.json(
        {
          error:
            backendJson.error ??
            "Lal Kitab calculation failed. Please verify birth details.",
        },
        { status: 502 }
      );
    }
    ...
    console.error("Lal Kitab proxy error:", e);
