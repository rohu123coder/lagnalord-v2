...
export async function POST(req: Request) {
  ...
  try {
    const backendRes = await fetch(`${BACKEND_URL}/api/kaal-sarp-dosha/calculate`, {
      method: "POST",
    ...
    if (!backendRes.ok) {
      ...
      console.error(
        "Backend kaal-sarp-dosha error:",
        backendRes.status,
        errText.slice(0, 500)
      );
      return NextResponse.json(
        { error: "Kaal Sarp calculation service unavailable. Please try again." },
        { status: 503 }
      );
    }
      ...
      return NextResponse.json(
        {
          error:
            backendJson.error ??
            "Kaal Sarp calculation failed. Please verify birth details.",
        },
        { status: 502 }
      );
    }
    ...
    console.error("Kaal Sarp proxy error:", e);
