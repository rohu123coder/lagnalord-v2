...
function dateToJD(year, month, day, hour, minute, utcOffset) {
...
export function calculateKundaliFromInput(input) {
    if (!swisseph) {
    ...
    const tobStr = typeof tob === "string" && tob.trim().length > 0 ? tob.trim().slice(0, 5) : undefined;
    if (tobStr) {
