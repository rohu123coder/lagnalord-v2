...
router.post("/test-send", async (req, res) => {
    try {
        ...
        const userResult = await pool.query(`SELECT id, name, expo_push_token, push_enabled
...
router.post("/register-token", async (req, res) => {
    try {
       ...
       SET expo_push_token = $1,
           ...
           push_enabled = true
       WHERE id = $3
       RETURNING id, name, push_platform, push_token_updated_at`, [token, platform ?? null, userId]);
