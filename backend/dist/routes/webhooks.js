...
async function findLeadForPayment(entity) {
    ...
    if (leadIdFromNotes) {
        const byId = await query(`SELECT id, name, email, phone, city, current_business, status
       FROM demo_booking_leads WHERE id = $1`, [leadIdFromNotes]);
        if (byId.rows[0])
            return byId.rows[0];
    }
    ...
    if (phone || email) {
        const byContact = await query(`SELECT id, name, email, phone, city, current_business, status
       ...
       LIMIT 1`, [phone ?? null, email ?? null]);
        if (byContact.rows[0])
            return byContact.rows[0];
    }
    return null;
}
async function markLeadPaid(lead, paymentId, amountPaise) {
    if (lead.status === "paid")
        return;
    ...
    await query(`UPDATE demo_booking_leads
     SET status = 'paid',
         ...
         amount_paid = $2,
         paid_at = now()
     WHERE id = $3`, [paymentId, amountPaid, lead.id]);
    ...
    try {
        if (event === "payment.captured") {
            ...
            if (lead && lead.status !== "paid") {
                await query(`UPDATE demo_booking_leads SET status = 'payment_failed' WHERE id = $1`, [lead.id]);
            }
        }
    }
    catch (err) {
