...
import { sendAdminDemoBookingNotification, sendDemoBookingConfirmation, } from "../lib/email.js";
const router = Router();
...
router.post("/demo-booking", async (req, res) => {
    ...
    const leadResult = await query(`INSERT INTO demo_booking_leads (name, email, phone, city, current_business, amount, source, status)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     RETURNING id`, [name, email, phone, city, currentBusiness ?? null, DEMO_AMOUNT_INR, source, status]);
...
router.post("/demo-booking/verify", async (req, res) => {
    ...
    const { leadId, razorpay_order_id, razorpay_payment_id, razorpay_signature } = parsed.data;
    ...
    const leadResult = await query(`SELECT id, name, email, phone, city, current_business, status, razorpay_order_id
     FROM demo_booking_leads WHERE id = $1`, [leadId]);
    ...
    await query(`UPDATE demo_booking_leads
     SET status = 'paid',
         razorpay_payment_id = $1,
         razorpay_order_id = COALESCE(razorpay_order_id, $2),
         amount_paid = $3,
         paid_at = now()
     WHERE id = $4`, [razorpay_payment_id, razorpay_order_id, DEMO_AMOUNT_INR, leadId]);
    void sendDemoBookingConfirmation(lead.email, lead.name).catch((err) => console.error("[Email] Demo confirmation failed:", err));
