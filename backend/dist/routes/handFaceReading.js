...
import { buildFaceReport, buildPalmReport, getVisionObservations, HAND_FACE_CATEGORIES, isValidHandFaceCategory, } from "../services/handFaceReadingService.js";
const router = Router();
...
function parseCategory(req, res) {
...
router.post("/palm", authMiddleware, readingUpload.single("palm"), async (req, res) => {
    ...
    let palmPhotoUrl;
    try {
        palmPhotoUrl = await uploadToCloudinary(palmFile.buffer, "divinemarg/readings/palm");
    }
    catch (e) {
    ...
    try {
        const insert = await query(`INSERT INTO palm_face_readings (
...
router.post("/face", authMiddleware, readingUpload.single("face"), async (req, res) => {
    if (rejectAstrologer(req, res))
        return;
    const userId = req.user?.userId;
    if (!userId) {
    ...
    let facePhotoUrl;
    try {
        facePhotoUrl = await uploadToCloudinary(faceFile.buffer, "divinemarg/readings/face");
    }
    catch (e) {
        console.error("[HandFaceReading] Cloudinary upload failed:", e);
        res.status(500).json({ success: false, error: "Photo upload failed" });
...
router.get("/", authMiddleware, async (req, res) => {
    ...
    try {
        const result = await query(`SELECT id, user_id, category, reading_type, palm_photo_url, face_photo_url,
              overview, samagri, vidhi, raw_observations, created_at
