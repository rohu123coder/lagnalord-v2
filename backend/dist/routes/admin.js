...
router.post("/login", async (req, res) => {
    ...
    const result = await query(`SELECT id, email, password_hash, role::text AS role
     FROM admins WHERE LOWER(email) = LOWER($1)`, [email]);
    ...
    const token = jwt.sign({ userId: admin.id, role: admin.role }, jwtSecret(), { expiresIn: "7d" });
    res.json({
        success: true,
        data: {
            token,
...
router.get("/stats", async (_req, res) => {
    const [usersCount, astrologersCount, revenueRow, sessionsCount] = await Promise.all([
        query(`SELECT COUNT(*)::text AS count FROM users`).then((r) => Number(r.rows[0]?.count ?? 0)),
        query(`SELECT COUNT(*)::text AS count FROM astrologers`).then((r) => Number(r.rows[0]?.count ?? 0)),
        query(`SELECT COALESCE(SUM(amount), 0)::text AS sum
         FROM transactions
         WHERE type = 'recharge' AND status = 'success'`).then((r) => Number(r.rows[0]?.sum ?? 0)),
        query(`SELECT COUNT(*)::text AS count FROM chat_sessions WHERE status = 'active'`).then((r) => Number(r.rows[0]?.count ?? 0)),
    ]);
    res.json({
        success: true,
        data: {
...
router.get("/astrologers", async (req, res) => {
    ...
    const countResult = await query(`SELECT COUNT(*)::text AS count
     ...
     WHERE TRUE ${searchFilter}`, countParams);
    const total = Number(countResult.rows[0]?.count ?? 0);
...
router.post("/astrologers/:id/verify", async (req, res) => {
    ...
    const updated = await query(`UPDATE astrologers SET is_verified = true WHERE id = $1 RETURNING id`, [id]);
    if (!updated.rows[0]) {
...
router.post("/astrologers/:id/suspend", async (req, res) => {
    const parsed = idParamSchema.safeParse(req.params);
    if (!parsed.success) {
        res.status(400).json({
            success: false,
    ...
    const { id } = parsed.data;
    const updated = await query(`UPDATE astrologers
...
router.get("/users", async (req, res) => {
    ...
    const countResult = await query(`SELECT COUNT(*)::text AS count FROM users u WHERE TRUE ${searchCond}`, searchParam);
    const total = Number(countResult.rows[0]?.count ?? 0);
...
router.post("/users/:id/suspend", async (req, res) => {
    ...
    const { id } = parsed.data;
    const updated = await query(`UPDATE users SET is_suspended = true WHERE id = $1 RETURNING id`, [id]);
    if (!updated.rows[0]) {
...
const settingsPutBody = z.record(z.string(), z.union([z.string(), z.number(), z.null()]));
router.put("/settings", async (req, res) => {
    ...
    try {
        ...
        for (const [key, value] of entries) {
            await client.query(`INSERT INTO platform_settings (key, value, updated_at)
         VALUES ($1, $2, now())
