...
import { authMiddleware, optionalAuthMiddleware, requireAstrologer, } from "../middleware/auth.js";
const router = Router();
...
function uploadToCloudinary(buffer) {
    return new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream({ folder: "divinemarg/profiles", resource_type: "image" }, (error, result) => {
            if (error || !result)
                return reject(error);
...
router.get("/", async (req, res) => {
    ...
    const conditions = ["(a.is_verified = true OR a.is_approved = true)"];
    const params = [];
    let p = 1;
    if (specialization) {
    ...
    const astrologers = result.rows.map((row) => ({
        ...
        avg_session_duration: row.avg_session_duration != null ? Number(row.avg_session_duration) : null,
        ...
        estimated_wait: Math.round(row.waiting_count *
...
router.get("/dashboard", authMiddleware, requireAstrologer, async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
    }
    const aResult = await query(`SELECT id, COALESCE(avg_rating, rating) AS rating, total_reviews, is_available, is_online,
              chat_available, voice_available, video_available
       FROM astrologers WHERE user_id = $1`, [userId]);
    const astrologer = aResult.rows[0];
    if (!astrologer) {
        res.status(404).json({ success: false, error: "Astrologer not found" });
        return;
    }
    const earningsResult = await query(`SELECT COALESCE(SUM(amount), 0)::text AS sum
       FROM astrologer_earnings_log
       WHERE astrologer_id = $1`, [astrologer.id]);
    const sessionsResult = await query(`SELECT COUNT(*)::text AS count
       FROM chat_sessions
       WHERE astrologer_id = $1 AND status = 'ended'`, [astrologer.id]);
    const todayResult = await query(`SELECT COALESCE(SUM(amount), 0)::text AS sum
       FROM astrologer_earnings_log
       WHERE astrologer_id = $1
         AND created_at >= date_trunc('day', CURRENT_TIMESTAMP)
         AND created_at < date_trunc('day', CURRENT_TIMESTAMP) + interval '1 day'`, [astrologer.id]);
    const monthResult = await query(`SELECT COALESCE(SUM(amount), 0)::text AS sum
       FROM astrologer_earnings_log
       WHERE astrologer_id = $1
         AND created_at >= date_trunc('month', CURRENT_TIMESTAMP)
         AND created_at < date_trunc('month', CURRENT_TIMESTAMP) + interval '1 month'`, [astrologer.id]);
    const weekRows = await query(`SELECT created_at, amount::text AS amount
       FROM astrologer_earnings_log
       WHERE astrologer_id = $1 AND created_at >= now() - interval '8 days'`, [astrologer.id]);
    const byDay = new Map();
    ...
    res.json({
        ...
        data: {
            ...
            rating: astrologer.rating != null ? Number(astrologer.rating) : null,
...
router.put("/profile", authMiddleware, requireAstrologer, async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
    ...
    params.push(userId);
    const result = await query(`UPDATE astrologers SET ${sets.join(", ")} WHERE user_id = $${i}
       RETURNING id, bio, specializations, languages, COALESCE(avg_rating, rating) AS rating, total_reviews,
         price_per_minute, is_available, is_verified, experience_years`, params);
    ...
    res.json({
        ...
        data: {
            astrologer: {
                ...row,
                rating: row.rating != null ? Number(row.rating) : null,
                price_per_minute: row.price_per_minute != null
...
async function updateAvailability(req, res) {
    ...
    const b = parsed.data;
    const online = typeof b.is_online === "boolean"
        ? b.is_online
    ...
    params.push(userId);
    const result = await query(`UPDATE astrologers SET ${sets.join(", ")}
     WHERE user_id = $${i}
     RETURNING is_available, is_online, chat_available, voice_available, video_available`, params);
    const row = result.rows[0];
    if (!row) {
        res.status(404).json({ success: false, error: "Astrologer not found" });
        return;
...
router.put("/availability", authMiddleware, requireAstrologer, updateAvailability);
router.patch("/availability", authMiddleware, requireAstrologer, updateAvailability);
router.post("/photo", authMiddleware, requireAstrologer, photoUpload.single("photo"), async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
    ...
    const aUp = await query(`UPDATE astrologers SET profile_photo_url = $1 WHERE user_id = $2`, [photoUrl, userId]);
    if (aUp.rowCount === 0) {
        res.status(404).json({ success: false, error: "Astrologer not found" });
        return;
    }
...
router.get("/:id", optionalAuthMiddleware, async (req, res) => {
    ...
    const reviewsResult = await query(`SELECT
       cs.id,
       cs.rating,
     ...
     LIMIT 30`, [id]);
    const ratingBreakdownResult = await query(`SELECT
    ...
    res.json({
        ...
        data: {
            astrologer: {
                ...
                voice_available: row.voice_available,
                video_available: row.video_available,
                profile_photo_url: row.profile_photo_url ?? row.user_profile_photo_url ?? null,
                waiting_count: row.waiting_count,
                estimated_wait: Math.round(row.waiting_count *
