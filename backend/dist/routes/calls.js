...
router.post("/request", async (req, res) => {
    ...
    const astroResult = await pool.query(`SELECT id, user_id, price_per_minute, is_available, is_online
     FROM astrologers WHERE id = $1`, [astrologer_id]);
    ...
    const walletResult = await pool.query(`SELECT wallet_balance FROM users WHERE id = $1`, [userId]);
    ...
    const sessionResult = await pool.query(`INSERT INTO chat_sessions (user_id, astrologer_id, status, session_type)
     VALUES ($1, $2, 'waiting', 'voice')
     RETURNING id`, [userId, astrologer_id]);
