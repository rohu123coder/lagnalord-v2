...
router.post("/request", requireUserRole, async (req, res) => {
    ...
    const problemAreaRaw = parsed.data.problem_area ?? parsed.data.problemArea ?? null;
    const problemAreaStr = Array.isArray(problemAreaRaw)
        ...
        : problemAreaRaw != null && String(problemAreaRaw).trim() !== ""
            ...
            : null;
    const astroResult = await query(`SELECT a.id, a.price_per_minute, a.is_verified, u.name
       FROM astrologers a
       INNER JOIN users u ON u.id = a.user_id
       WHERE a.id = $1`, [astrologer_id]);
    const astro = astroResult.rows[0];
    if (!astro || !astro.is_verified) {
        res.status(404).json({ success: false, error: "Astrologer not found" });
        return;
    }
    const activeSessionResult = await query(`SELECT id
       FROM chat_sessions
       WHERE astrologer_id = $1
         AND status = 'active'
       LIMIT 1`, [astrologer_id]);
    if (activeSessionResult.rows[0]) {
        const queueResult = await query(`SELECT COUNT(*)::text AS count
         FROM astrologer_waitlist
         WHERE astrologer_id = $1
           AND status = 'waiting'`, [astrologer_id]);
    ...
    const walletResult = await query(`SELECT wallet_balance FROM users WHERE id = $1`, [userId]);
    ...
    const sessionResult = await query(`INSERT INTO chat_sessions (user_id, astrologer_id, status, started_at, problem_area)
       VALUES ($1, $2, 'waiting', now(), $3)
       RETURNING id`, [userId, astrologer_id, problemAreaStr]);
    const session = sessionResult.rows[0];
    if (!session) {
        res.status(500).json({ success: false, error: "Could not create session" });
        return;
    }
    void autoInjectIntroMessage(session.id, userId, astro.name, problemAreaStr).catch((err) => console.error("[Auto-Inject] Error:", err));
    const notifyResult = await query(`SELECT a.user_id AS astrologer_user_id, u.name AS user_name
       FROM astrologers a
       INNER JOIN users u ON u.id = $2
       WHERE a.id = $1`, [astrologer_id, userId]);
...
router.post("/end/:sessionId", requireUserRole, async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
    ...
    try {
        ...
        const lockResult = await client.query(`SELECT cs.id, cs.user_id, cs.astrologer_id, cs.status, cs.started_at,
                a.price_per_minute
        ...
        await client.query(`UPDATE chat_sessions
         ...
         WHERE id = $3`, [totalMinutes, totalCharged, sessionId]);
...
router.get("/history", async (req, res) => {
    ...
    if (role === "astrologer") {
       ...
       FROM chat_sessions cs
       INNER JOIN astrologers a ON a.id = cs.astrologer_id
       WHERE a.user_id = $1 AND cs.status = 'ended'`, [userId]);
        ...
        res.json({
            ...
            data: {
                sessions: result.rows.map((s) => ({
                    ...s,
                    total_charged: s.total_charged != null ? Number(s.total_charged) : null,
                })),
                page,
                limit,
                total,
    ...
    const countResult = await query(`SELECT COUNT(*)::text AS count FROM chat_sessions WHERE user_id = $1`, [userId]);
    const total = Number(countResult.rows[0]?.count ?? 0);
    const result = await query(`SELECT
       cs.id,
       cs.astrologer_id,
...
router.get("/history/:sessionId/messages", async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
    ...
    const sessionId = idParse.data;
    const accessCheck = await query(`SELECT cs.id
     FROM chat_sessions cs
     INNER JOIN astrologers a ON a.id = cs.astrologer_id
    ...
    const messagesResult = await query(`SELECT m.id,
            m.sender_id,
