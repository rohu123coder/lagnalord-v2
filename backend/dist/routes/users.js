...
router.put("/profile", async (req, res) => {
    ...
    if (sets.length === 0) {
        const existing = await query(`SELECT id, name, email, phone, avatar_url, wallet_balance, created_at
       FROM users WHERE id = $1`, [userId]);
    ...
    params.push(userId);
    const whereParam = `$${i}`;
    const result = await query(`UPDATE users SET ${sets.join(", ")} WHERE id = ${whereParam}
     RETURNING id, name, email, phone, avatar_url, wallet_balance, created_at`, params);
    const row = result.rows[0];
    if (!row) {
        res.status(404).json({ success: false, error: "User not found" });
        return;
...
router.get("/wallet", async (req, res) => {
    ...
    const txResult = await query(`SELECT id, type, amount, status, created_at, razorpay_order_id, razorpay_payment_id
     FROM transactions
     WHERE user_id = $1
...
router.patch("/birth-details", async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
    ...
    const { dateOfBirth, timeOfBirth, placeName, lat, lng, utcOffset, gender, maritalStatus, occupation, } = parsed.data;
    const utc = utcOffset ?? 5.5;
    const tob = timeOfBirth != null && String(timeOfBirth).trim() !== ""
...
router.get("/birth-details", async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
       ...
       FROM users WHERE id = $1`, [userId]);
        ...
        const dob = row.date_of_birth instanceof Date
            ...
            : row.date_of_birth
                ...
                : null;
        const tobOut = row.time_of_birth != null
        ...
        res.json({
            ...
            data: {
                ...
                utcOffset: row.birth_utc_offset != null
