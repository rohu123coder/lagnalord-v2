...
router.post("/create-order", async (req, res) => {
    ...
    const receipt = `rc_${userId.replace(/-/g, "").slice(0, 12)}_${Date.now()}`.slice(0, 40);
    let order;
    try {
    ...
    await query(`INSERT INTO transactions (user_id, type, amount, razorpay_order_id, status)
     VALUES ($1, 'recharge', $2, $3, 'pending')`, [userId, amount, order.id]);
    res.status(201).json({
        success: true,
        data: {
            orderId: order.id,
...
router.post("/verify-payment", async (req, res) => {
    ...
    try {
        ...
        const txResult = await client.query(`SELECT id, amount, status FROM transactions
       WHERE razorpay_order_id = $1 AND user_id = $2
       FOR UPDATE`, [razorpay_order_id, userId]);
       ...
       WHERE id = $2`, [creditAmount, userId]);
        await client.query(`UPDATE transactions
       SET status = 'success',
           razorpay_payment_id = $1
       WHERE id = $2`, [razorpay_payment_id, tx.id]);
        await client.query("COMMIT");
        const balResult = await query(`SELECT wallet_balance FROM users WHERE id = $1`, [userId]);
...
router.get("/transactions", async (req, res) => {
    ...
    const countResult = await query(`SELECT COUNT(*)::text AS count FROM transactions WHERE user_id = $1`, [userId]);
    const total = Number(countResult.rows[0]?.count ?? 0);
    const result = await query(`SELECT id, type, amount, status, created_at, razorpay_order_id, razorpay_payment_id
     FROM transactions
     WHERE user_id = $1
