...
router.get("/:sessionId/context", async (req, res) => {
    ...
    const row = await pool.query(`SELECT u.name AS astrologer_name,
       ...
       WHERE cs.id = $1 AND cs.user_id = $2`, [sessionId, userId]);
...
router.get("/:sessionId/call-token", async (req, res) => {
    const userId = req.user?.userId;
    if (!userId) {
        res.status(401).json({ success: false, error: "Unauthorized" });
        return;
    ...
    const sessionRow = await pool.query(`SELECT cs.id,
              cs.status,
              cs.session_type,
              cs.user_id,
    ...
    await pool.query(`UPDATE chat_sessions
       ...
       WHERE id = $2`, [channelName, sessionId]);
...
router.post("/:sessionId/rate", async (req, res) => {
    ...
    try {
        ...
        const sessionResult = await client.query(`SELECT id, astrologer_id, user_id, rated_at, status
       FROM chat_sessions
       WHERE id = $1
       ...
       SET rating = $1,
           review_text = $2,
           rated_at = NOW()
       WHERE id = $3`, [rating, reviewText?.trim() || null, sessionId]);
       ...
       WHERE id = $1`, [session.astrologer_id]);
