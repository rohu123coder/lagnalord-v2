...
const CATEGORY_TEMPLATES = {
    "Career and Business": {
        ...
        samagri: () => "1 piece Citrine ya Yellow Sapphire (as per budget), 1 small brass diya, desi ghee, 5 yellow flowers, 1 yellow cloth, aur ek copper coin. In sab ko ek chhote thali mein rakho.",
        vidhi: () => "Har Thursday subah 6–8 baje ke beech, pehle haath-dho kar yellow cloth par baith jao. Citrine ko right haath mein pakad kar 11 baar 'Om Gurave Namah' bolo. Phir diya jalao aur 5 minute tak career growth ka sankalp lo. 21 din tak repeat karo — best results ke liye same time follow karo.",
    },
    "Love and Relationship": {
        palmOverview: (ctx) => {
            const visual = palmPhrase(ctx.palm);
        ...
        samagri: () => "Gulab ki 5 pankhudiyan, 1 chhoti Rose Quartz mala ya stone, shakkar, 1 white cloth, aur ek chhota silver bowl (steel bhi chalega). Paani thoda sa alag rakho.",
        vidhi: () => "Har Friday shaam ko, white cloth par baith kar gulab ki pankhudiyan bowl mein rakho. Rose Quartz ko dil ke paas rakh kar 11 baar 'Om Kamadevaya Namah' whisper karo. Phir thoda paani aur shakkar mila kar apne aas-paas chhida do (ya plant ko de do). 11 din tak karo — is duration mein negative thoughts ko feed mat karo.",
    },
    Marriage: {
        palmOverview: (ctx) => {
            const visual = palmPhrase(ctx.palm);
        ...
        samagri: () => "2 laal chunri ya laal kapda ke tukde, 1 mangalsutra-style thread (simple red-yellow thread bhi chalega), chhoti mithai, 1 diya, desi ghee, aur haldi powder thoda sa.",
        vidhi: () => "Har Tuesday ya Friday ko shaam 6 baje ke baad, laal kapde par baith kar diya jalao. Thread ko dono haathon mein lapet kar 11 baar 'Om Gauri Shankaraya Namah' bolo. Mithai ka ek tukda apne saamne rakh kar marriage harmony ka sankalp lo. 21 din tak — specially jhagda wale din skip mat karo, bas calmly karo.",
    },
    "Financial Problems": {
        palmOverview: (ctx) => {
            const visual = palmPhrase(ctx.palm);
        ...
        samagri: () => "1 Kuber Yantra (chhota print bhi chalega), 1 green cloth, 11 rice grains, 1 copper coin, aur thoda ilaichi powder. Ek chhota kalash ya bowl rakho.",
        vidhi: () => "Har Wednesday subah, green cloth par Kuber Yantra rakho. 11 rice grains aur coin saath mein rakh kar 11 baar 'Om Shreem Mahalakshmiyei Namah' bolo. Us din ek unnecessary expense consciously avoid karo. 21 din tak repeat — har hafte apne accounts ek baar review karo.",
    },
    "Family Issues": {
        palmOverview: (ctx) => {
            const visual = palmPhrase(ctx.palm);
        ...
        samagri: () => "1 chhoti family photo (print), 1 white cloth, chandan powder, 1 diya, desi ghee, aur 5 tulsi ya neem patte (tulsi na ho toh koi bhi green patte).",
        vidhi: () => "Har Sunday subah, family photo ko white cloth par rakho. Chandan ka chhota tilak photo par lagao (ya photo ke saamne lagao). Diya jalakar 11 baar 'Om Namah Shivaya' bolo aur family peace ka sankalp lo. 11 din tak — is duration mein ghar mein intentionally ek positive baat share karo har din.",
    },
    Health: {
        palmOverview: (ctx) => {
            const visual = palmPhrase(ctx.palm);
        ...
        samagri: () => "1 amethyst ya clear quartz (chhota), 1 blue cloth, paani ka glass, thoda sendha namak, aur ek fresh fruit (seasonal — apple ya banana chalega).",
        vidhi: () => "Har subah khali pet paani mein thoda sendha namak mila kar peene se pehle, blue cloth par baith kar stone ko pakad kar 11 baar 'Om Dhanvantre Namah' bolo. Us din ek fresh fruit khud ko gift karo. 21 din tak — sath mein 20 minute walk ya light stretch add karo agar doctor allow kare.",
    },
    Education: {
        palmOverview: (ctx) => {
            const visual = palmPhrase(ctx.palm);
        ...
        samagri: () => "1 yellow notebook ya study diary, 1 pen, 1 Saraswati yantra (chhota print), yellow flowers 3, aur ek chhota misri ka tukda.",
        vidhi: () => "Har subah padhai se pehle, diary aur pen saamne rakho. 11 baar 'Om Saraswatyai Namah' bolo aur aaj ka ek clear study goal likho. Misri muh mein rakh kar padhai shuru karo. 21 din tak — har raat 5 minute revise karo jo seekha. Exam ke din extra calm rehna.",
    },
    Other: {
        palmOverview: (ctx) => {
            const visual = palmPhrase(ctx.palm);
        ...
        samagri: () => "1 white candle ya diya, 1 multi-colour thread (7 colours wala rakhi thread bhi chalega), chhoti mithai, aur ek chhota notebook.",
        vidhi: () => "Har Monday ko subah, notebook mein apna concern ek line mein likho. Thread ko wrist par 3 baar lapet kar 11 baar 'Om Namah Shivaya' bolo. Diya/candle jalao aur 5 minute shant baitho. 11 din tak — har din ek chhota positive action lo jo concern se related ho.",
    },
};
async function analyzePalmImage(imageUrl) {
...
async function analyzeFaceImage(imageUrl) {
    const client = await getAnthropicClient();
    if (!client) {
...
function parsePalmObservationString(raw) {
    try {
...
function parseFaceObservationString(raw) {
    try {
        return JSON.parse(raw);
    }
    catch {
...
function resolveCategoryKey(category) {
    return HAND_FACE_CATEGORIES.includes(category)
        ? category
        : "Other";
}
export function buildPalmReport(category, palmObservations) {
...
export function buildFaceReport(category, faceObservations) {
    const templates = CATEGORY_TEMPLATES[resolveCategoryKey(category)];
...
export function isValidHandFaceCategory(value) {
    return HAND_FACE_CATEGORIES.includes(value);
}
