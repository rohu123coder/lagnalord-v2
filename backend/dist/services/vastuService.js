export const VASTU_DIRECTIONS = ["N", "NE", "E", "SE", "S", "SW", "W", "NW", "Center"];
export const VASTU_PROPERTY_TYPES = ["residential", "apartment", "office", "shop"];
...
function getPhotoScoreCap(photoCount) {
    if (photoCount === 1)
        return 70;
    if (photoCount <= 3)
        return 80;
...
export const VASTU_RULES = [
    {
        ...
        scientificReason: "Entrance direction determines solar energy intake — East/North entrances receive morning sun (positive ions), South/SW entrances receive afternoon heat causing stress and financial instability",
        traditionalReason: "Per Vastu Purush Mandala, SW is governed by Nairutya (demon energy), WNW by Roga (disease deity) — main door here invites negative cosmic forces",
        remedy: {
    ...
    },
    {
        ...
        room: "bedroom",
        severity: "critical",
        problem: "Master bedroom in wrong zone causes relationship issues, health problems, and financial instability for head of family",
        scientificReason: "SW zone has maximum earth element stability — heavy mass in SW keeps the house grounded. NE is water/clarity zone, not suitable for sleep as it causes restlessness",
        traditionalReason: "SW is Pitra/Dauvarik zone governed by ancestors — master bedroom here ensures family head has authority and stability",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "SE is the zone of fire element (Agni) — cooking here aligns with natural fire energy direction. Kitchen in NE (water zone) creates fire-water conflict causing digestive issues and financial loss",
        traditionalReason: "SE is governed by Anil/Vitatha deities associated with fire and digestion. NE is Shikhi/Parjanya zone — fire here destroys clarity and causes confusion",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "NE receives maximum cosmic radiation in morning — toilet here pollutes the most spiritually active zone. SW toilet drains earth energy causing instability",
        traditionalReason: "NE is Ishanya zone — the most sacred corner governed by Lord Shiva. Toilet here is the gravest Vastu defect",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "WNW and ESE zones are neutral zones suitable for utility appliances with water and movement",
        traditionalReason: "Washing machine represents Vayu (air/movement) + Jal (water) — WNW is governed by Vayu energy making it ideal",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "Waste accumulation creates bacterial growth and negative environmental energy regardless of direction",
        traditionalReason: "Dustbin represents decay (Nirrti energy) — keeping it covered and in SSW or WNW zones is preferred",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "NNE is governed by Aditi/Diti deities associated with immunity and healing. North zone has optimal magnetic north alignment supporting healing",
        traditionalReason: "NNE is the zone of Dhanvantari (Ayurvedic deity) — medicines here receive maximum healing cosmic energy",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "NE receives maximum morning sun and cosmic radiation — ideal for meditation and prayer. Sharing wall with toilet creates contamination of sacred space",
        traditionalReason: "NE is Ishanya zone governed by Lord Shiva/Brahma — the most sacred corner of any property",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "North is magnetic north — facing North maximizes positive ionic flow toward valuables. Kuber (wealth deity) faces North",
        traditionalReason: "North is Kubera zone — god of wealth. Safe facing North ensures Kubera blesses the wealth stored",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "SE zone is fire element zone — electrical equipment generates heat/fire energy which is amplified positively in SE",
    ...
    },
    {
        ...
        scientificReason: "East-facing study gets morning sun which activates pineal gland improving alertness. North-facing study aligns with magnetic north improving memory and focus",
        traditionalReason: "WSW is Vastu zone of knowledge (Vithi/Gandharva). NE is clarity zone — students studying here gain exceptional focus",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "West-facing dining aligns with evening energy suitable for family meals. SE (fire zone) causes over-eating or digestive fire imbalance",
        traditionalReason: "West is Varuna zone governing water and sustenance. Dining here ensures nourishment and family bonding",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "TV emits EMF radiation — NE is most sensitive zone for brain activity. TV here disrupts mental clarity and spiritual development",
        traditionalReason: "NE is clarity/mind zone — electronic disturbances here block divine signals and mental peace",
        remedy: {
    ...
    },
    {
        ...
        traditionalReason: "NW is Vayu zone governing movement and transience — ideal for guests",
        remedy: {
    ...
    },
    {
        ...
        room: "bedroom",
        severity: "high",
        problem: "Mirror in wrong direction or reflecting bed causes sleep disturbances, health issues, and relationship problems",
        scientificReason: "Mirrors reflecting beds disturb REM sleep cycles by creating psychological alertness. South-facing mirrors reflect and amplify heat energy",
        traditionalReason: "Mirror reflecting bed doubles illness and marital discord per Vastu — it is considered highly inauspicious",
        remedy: {
    ...
    },
    {
        ...
        scientificReason: "East-facing plants receive morning photosynthesis energy producing maximum oxygen. SW plants compete with earth element causing instability",
    ...
    },
    {
        ...
        scientificReason: "NE and North need to be light and open for energy circulation. Heavy mass here creates stagnation in career and mental zones",
        traditionalReason: "SW is earth element zone — heavy objects here strengthen foundation and stability. NE with heavy objects suffocates clarity zone",
        remedy: {
...
export const ZONE_ACTIVATION_GUIDE = {
    N: {
        fullName: "North",
...
function normalizeZoneLabel(zone) {
    ...
    if (ZONE_NAME_ALIASES[upper])
        return ZONE_NAME_ALIASES[upper];
    if (upper === "SHARING_TOILET_WALL")
        return "sharing_toilet_wall";
    const subMatch = trimmed.match(/^([NSEW]{1,3})(\d+)$/i);
    if (subMatch)
        return `${subMatch[1].toUpperCase()}${subMatch[2]}`;
    return trimmed.length <= 3 ? upper : trimmed;
}
function getZoneParent(zone) {
...
function zoneMatches(detectedZones, ruleZone) {
    const ruleNormalized = normalizeZoneLabel(ruleZone);
    if (ruleNormalized === "sharing_toilet_wall")
        return false;
    for (const detected of detectedZones) {
        const dn = normalizeZoneLabel(detected);
        if (dn === ruleNormalized)
            return true;
        const detectedParent = getZoneParent(dn);
        const ruleParent = getZoneParent(ruleNormalized);
        if (detectedParent && ruleParent && detectedParent === ruleParent)
            return true;
        if (detectedParent === ruleNormalized || dn === ruleParent)
            return true;
        if (ruleNormalized === "N" && (dn === "N" || dn.startsWith("N")))
            return true;
        if (ruleNormalized === "S" && (dn === "S" || dn.startsWith("S")))
            return true;
        if (ruleNormalized === "E" && (dn === "E" || dn.startsWith("E")))
            return true;
        if (ruleNormalized === "W" && (dn === "W" || dn.startsWith("W")))
            return true;
    }
    return false;
}
export function degreesToDirection16(degrees, northDirection) {
...
export function degreesToSubZone(degrees, northDirection) {
    const adjusted = ((degrees - northDirection) % 360 + 360) % 360;
...
function resolveZones(direction, degrees, northDirection) {
    const zones = new Set();
...
function parseZoneFromText(text) {
...
function evaluateRulePlacement(rule, detectedZones) {
    const zoneLabel = detectedZones[0] ?? "unknown";
    const isStrictViolation = rule.strictNoZones.some((z) => zoneMatches(detectedZones, z));
    const isRemedyViolation = !isStrictViolation && rule.remedyZones.some((z) => zoneMatches(detectedZones, z));
    const isIdeal = rule.idealZones.some((z) => zoneMatches(detectedZones, z));
    const isBeneficial = !isIdeal && rule.beneficialZones.some((z) => zoneMatches(detectedZones, z));
    return {
...
function matchObjectToRuleObject(detectedName) {
    ...
    if (VASTU_OBJECTS_COVERED.includes(norm))
        return norm;
    return null;
}
function ruleAppliesToProperty(rule, propertyType) {
    if (!rule.propertyTypes || rule.propertyTypes.length === 0)
        return true;
    return rule.propertyTypes.includes(propertyType);
}
export function applyVastuRules(analysis, propertyType) {
    const applied = [];
    const seen = new Set();
    for (const room of analysis.detectedRooms) {
        ...
        for (const obj of room.detectedObjects) {
            ...
            if (ruleObject) {
                const relevantRules = VASTU_RULES.filter((r) => r.object === ruleObject &&
                ...
                for (const rule of relevantRules) {
                    ...
                    if (!placement.isViolation && !placement.isIdeal && !placement.isBeneficial)
                        continue;
                    ...
                    if (seen.has(key))
                        continue;
                    seen.add(key);
                    const isRemedyOnly = placement.isViolation &&
...
function applyPhotoCountAdjustments(rawScore, photoCount) {
...
export function calculateVastuScore(analysis, appliedRules, photoCount) {
    ...
    const positiveEnergyZones = [];
    const negativeEnergyZones = [];
    const violations = appliedRules.filter((a) => a.isViolation);
    const hasUserDescriptions = analysis.detectedRooms.some((r) => r.userDescription && r.userDescription.trim().length > 0);
    for (const applied of appliedRules) {
        ...
        if (isViolation) {
            ...
            if (FINANCIAL_OBJECTS.has(rule.object))
                financialScore -= deduction;
            if (HEALTH_OBJECTS.has(rule.object))
                healthScore -= deduction;
            if (RELATIONSHIP_OBJECTS.has(rule.object))
                relationshipScore -= deduction;
            if (CAREER_OBJECTS.has(rule.object))
                careerScore -= deduction;
            ...
            if (FINANCIAL_OBJECTS.has(rule.object))
                financialScore += bonus;
            if (HEALTH_OBJECTS.has(rule.object))
                healthScore += bonus;
            if (RELATIONSHIP_OBJECTS.has(rule.object))
                relationshipScore += bonus;
            if (CAREER_OBJECTS.has(rule.object))
                careerScore += bonus;
    ...
    for (const room of analysis.detectedRooms) {
        if (room.naturalLight === "good")
            overallScore += 1;
        if (room.naturalLight === "poor")
            overallScore -= 2;
        if (room.ventilation === "good")
            overallScore += 1;
        if (room.ventilation === "poor")
            healthScore -= 3;
    }
    ...
    for (const key of Object.keys(elementBalance)) {
...
function buildIssues(appliedRules) {
    return appliedRules
...
function buildRoomAnalyses(analysis, appliedRules) {
    return analysis.detectedRooms.map((room) => {
        ...
        for (const a of roomApplied) {
            if (a.isViolation) {
                ...
                positives.push(`${a.detectedObject ?? a.rule.object.replace(/_/g, " ")} well placed in ${a.direction}`);
            }
        }
        if (room.naturalLight === "good") {
            score += 2;
        ...
        if (room.ventilation === "good")
            positives.push("Adequate ventilation maintains fresh prana");
        return {
...
function buildPanchtattvaAnalysis(scores, appliedRules) {
    const mapElement = (element, name, zones) => ({
        score: scores.elementBalance[element],
        zones,
        recommendation: scores.elementBalance[element] >= 75
...
function analysisVentilationZones(appliedRules) {
    return appliedRules
        .filter((a) => a.rule.object === "windows" || a.rule.object === "balcony")
        .map((a) => `${a.room} (${a.direction})`);
}
function buildRemedySummary(issues) {
    const immediate = [];
    const shortTerm = [];
    const longTerm = [];
    for (const issue of issues.slice(0, 10)) {
        ...
        if (issue.severity === "critical" || issue.severity === "high") {
            longTerm.push(`Plan structural correction for ${issue.object.replace(/_/g, " ")} in ${issue.room} (${issue.direction})`);
        }
    }
    return {
...
export function buildVastuReport(scores, appliedRules, analysis, propertyType) {
    ...
    const topPriorityFixes = issues
        .slice(0, 5)
        .map((i) => `[${i.severity.toUpperCase()}] ${i.object.replace(/_/g, " ")} in ${i.room} (${i.direction}): ${i.remedy.easy}`);
    const summaryLines = [
    ...
    const neutralZones = VASTU_DIRECTIONS.filter((d) => !scores.positiveEnergyZones.some((z) => z.includes(`(${d})`)) &&
        !scores.negativeEnergyZones.some((z) => z.includes(`(${d})`))).map((d) => `${d} zone — neutral, maintain cleanliness`);
    return {
        ...
        professionalNote: "This AI Vastu analysis combines traditional Vastu Shastra principles with spatial science observations from your photos. " +
...
function normalizeClaudeRoom(room, index, photos, northDirection) {
    const photo = photos[room.photoIndex ?? index];
    const compassDegrees = room.degrees ?? photo?.compassDirection ?? northDirection;
    const compassDirection = typeof room.compassDirection === "string"
...
function parseVastuAnalysisJson(text, photos, northDirection) {
    try {
        const parsed = JSON.parse(stripJsonFences(text));
        if (parsed?.detectedRooms && Array.isArray(parsed.detectedRooms)) {
            return {
                detectedRooms: parsed.detectedRooms.map((room, index) => normalizeClaudeRoom(room, index, photos, northDirection)),
            };
        }
    }
    catch {
...
export async function analyzeVastuPhotos(photos, propertyType, northDirection) {
    const client = await getAnthropicClient();
    if (!client || photos.length === 0) {
        return { ...FALLBACK_ANALYSIS };
    }
    const enrichedPhotos = photos.map((photo, i) => ({
        ...photo,
        userDescription: photo.userDescription ?? "",
        vastuDirection: degreesToDirection(photo.compassDirection, northDirection),
    }));
    const photoContext = enrichedPhotos
        ...
        .join("\n");
    const imageBlocks = enrichedPhotos.flatMap((photo, index) => [
        {
            type: "text",
            text: `--- Photo index ${index}: ${photo.roomLabel} (${photo.vastuDirection}, ${photo.compassDirection}°) — User says: "${photo.userDescription || "no description"}" ---`,
        },
        {
            type: "image",
            ...
            messages: [
                {
                    ...
                    content: [
                        ...
                        {
                            type: "text",
                            text: `Analyze this ${propertyType} property for Vastu compliance.
...
Analyze all ${photos.length} photos. Use user descriptions to enhance room identification and object detection.
Be STRICT — find real issues, flag unverified items, and return comprehensive JSON only.`,
                        },
                    ],
                },
            ],
        ...
        return parseVastuAnalysisJson(text, enrichedPhotos, northDirection);
    }
    catch (e) {
...
export function getVastuRulesCount() {
    return VASTU_RULES.length;
}
export function isValidPropertyType(value) {
    return VASTU_PROPERTY_TYPES.includes(value);
}
