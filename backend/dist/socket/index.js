...
async function loadSessionRow(sessionId) {
    const r = await pool.query(`SELECT cs.id,
            cs.user_id,
            cs.astrologer_id,
...
const waitingSessionTimeouts = new Map();
const pendingCalls = new Map();
async function loadCallSessionRow(sessionId) {
    const r = await pool.query(`SELECT cs.id,
            cs.status,
            cs.user_id,
            a.user_id AS astrologer_user_id,
...
function ensureLiveSession(sessionId, startTime) {
...
export function noteAutomatedUserIntro(sessionId) {
    const state = ensureLiveSession(sessionId);
    state.userMessageCount = (state.userMessageCount ?? 0) + 1;
}
function clearWaitingSessionTimeout(sessionId) {
...
async function loadAstrologerQueue(astrologerId) {
    const queueResult = await pool.query(`SELECT w.id,
            w.astrologer_id,
            w.user_id,
            w.position,
            u.name AS user_name
     ...
     ORDER BY w.position`, [astrologerId]);
    return queueResult.rows;
}
async function emitQueuePositionUpdates(io, astrologerId) {
    const result = await pool.query(`SELECT id, user_id, position
     FROM astrologer_waitlist
     WHERE astrologer_id = $1
       AND status = 'waiting'
     ORDER BY position`, [astrologerId]);
...
async function emitWaitlistUpdated(io, astrologerUserId, astrologerId) {
...
async function updateAstrologerAverageSessionDuration(client, astrologerId) {
    await client.query(`UPDATE astrologers
     ...
     WHERE id = $1`, [astrologerId]);
}
async function reorderWaitlistAfterRemoval(client, astrologerId, removedPosition) {
    await client.query(`UPDATE astrologer_waitlist
     SET position = position - 1,
         updated_at = now()
     WHERE astrologer_id = $1
       AND status = 'waiting'
       AND position > $2`, [astrologerId, removedPosition]);
}
async function acceptWaitlistEntry(io, astrologerUserId, waitlistId) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const waitlistResult = await client.query(`SELECT w.id,
              w.astrologer_id,
              w.user_id,
              w.position,
              w.status,
       ...
       WHERE w.id = $1
         AND a.user_id = $2
       FOR UPDATE`, [waitlistId, astrologerUserId]);
        const wait = waitlistResult.rows[0];
        if (!wait || wait.status !== "waiting") {
            await client.query("ROLLBACK");
            return { ok: false, reason: "waitlist_not_available" };
        }
        const activeSession = await client.query(`SELECT id
       FROM chat_sessions
       WHERE astrologer_id = $1
         AND status = 'active'
       LIMIT 1`, [wait.astrologer_id]);
        if (activeSession.rows[0]) {
            await client.query("ROLLBACK");
            return { ok: false, reason: "astrologer_busy" };
        }
        const sessionInsert = await client.query(`INSERT INTO chat_sessions (user_id, astrologer_id, status, started_at)
       VALUES ($1, $2, 'active', now())
       RETURNING id`, [wait.user_id, wait.astrologer_id]);
...
async function autoConnectNextInQueue(io, astrologerId, astrologerUserId, astrologerName) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const nextInQueue = await client.query(`SELECT w.id,
              w.astrologer_id,
              w.user_id,
              w.position,
              u.name AS user_name
       ...
       LIMIT 1
       FOR UPDATE`, [astrologerId]);
        const next = nextInQueue.rows[0];
        if (!next) {
            await client.query("ROLLBACK");
            return;
        }
        const newSession = await client.query(`INSERT INTO chat_sessions (user_id, astrologer_id, status, started_at)
       VALUES ($1, $2, 'active', NOW())
       RETURNING id`, [next.user_id, astrologerId]);
       ...
       SET status = 'accepted',
           session_id = $2,
           updated_at = now()
       WHERE id = $1`, [next.id, newSessionId]);
...
async function autoCancelWaitingSession(io, sessionId) {
    clearWaitingSessionTimeout(sessionId);
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const lockResult = await client.query(`SELECT cs.id,
              cs.user_id,
              a.user_id AS astrologer_user_id,
              au.name AS astrologer_name,
              cs.status
       ...
       SET status = 'ended',
           ...
           total_minutes = 0,
           total_charged = 0
       WHERE id = $1 AND status = 'waiting'`, [sessionId]);
        await client.query("COMMIT");
...
function ensureWaitingSessionTimeout(io, sessionId) {
...
function emitSessionTick(io, sessionId) {
...
function startSessionTimer(io, sessionId, startTime) {
...
async function finalizeChatSession(io, sessionId) {
    ...
    try {
       ...
       FOR UPDATE`, [sessionId]);
        ...
        const authoritativeStart = live?.startTime ??
            (row.started_at ? new Date(row.started_at).getTime() : Date.now());
        const rawDuration = Math.max(0, Math.ceil((Date.now() - authoritativeStart) / 60_000));
        const bothCommunicated = (live?.userMessageCount ?? 0) >= 1 &&
        ...
        if (totalCharged > 0) {
            const deduct = await client.query(`UPDATE users
         SET wallet_balance = wallet_balance - $1::numeric
         WHERE id = $2 AND wallet_balance >= $1::numeric
         RETURNING wallet_balance`, [totalCharged, userId]);
            ...
            await client.query(`INSERT INTO transactions (user_id, type, amount, status)
         VALUES ($1, 'deduction', $2, 'success')`, [userId, totalCharged]);
            await client.query(`INSERT INTO astrologer_earnings_log (astrologer_id, session_id, amount)
         VALUES ($1, $2, $3)`, [row.astrologer_id, sessionId, totalCharged]);
        }
        await client.query(`UPDATE chat_sessions
       SET status = 'ended',
           ended_at = now(),
       ...
       WHERE id = $3`, [effectiveDuration, totalCharged, sessionId]);
        ...
        console.log("[BILLING AUDIT]", JSON.stringify({
            sessionId,
            userId,
            astrologerName,
        ...
        await autoConnectNextInQueue(io, row.astrologer_id, row.astrologer_user_id, row.astrologer_name);
...
async function billingTick(io) {
    const result = await pool.query(`SELECT cs.id, u.wallet_balance, a.price_per_minute
     FROM chat_sessions cs
     INNER JOIN users u ON u.id = cs.user_id
     INNER JOIN astrologers a ON a.id = cs.astrologer_id
...
export function registerSocketHandlers(io) {
    ...
    setInterval(() => {
        void billingTick(io).catch((err) => console.error("billingTick failed:", err));
    }, 60_000);
    io.on("connection", (socket) => {
        ...
        if (socket.data.role === "astrologer") {
            void (async () => {
                const astroResult = await pool.query(`SELECT id FROM astrologers WHERE user_id = $1 LIMIT 1`, [user.userId]);
        ...
        socket.on("join_waitlist", async (payload) => {
         ...
         WHERE a.id = $1`, [astrologerId, user.userId]);
            const astro = astroResult.rows[0];
            if (!astro) {
                socket.emit("waitlist_error", { reason: "astrologer_not_found" });
                return;
            }
            const activeResult = await pool.query(`SELECT id
         FROM chat_sessions
         WHERE astrologer_id = $1
           AND status = 'active'
         LIMIT 1`, [astrologerId]);
            ...
            const existingResult = await pool.query(`SELECT id, position
         FROM astrologer_waitlist
         WHERE astrologer_id = $1
           AND user_id = $2
           AND status = 'waiting'
         LIMIT 1`, [astrologerId, user.userId]);
            ...
            const insertResult = await pool.query(`WITH next_pos AS (
         ...
         RETURNING id, position`, [astrologerId, user.userId]);
        ...
        socket.on("waitlist_action", async (payload) => {
            ...
            try {
                await client.query("BEGIN");
                const rowResult = await client.query(`SELECT w.id,
                  w.astrologer_id,
                  w.user_id,
                  w.position,
                  a.user_id AS astrologer_user_id,
           ...
           WHERE w.id = $1
             AND a.user_id = $2
           FOR UPDATE`, [waitlistId, user.userId]);
        ...
        socket.on("accept_from_waitlist", async (payload) => {
        ...
        socket.on("cancel_waitlist", async (payload) => {
        ...
        socket.on("get_waitlist", async () => {
            ...
            const astroResult = await pool.query(`SELECT id AS astrologer_id
         FROM astrologers
         WHERE user_id = $1
         LIMIT 1`, [user.userId]);
            const astro = astroResult.rows[0];
        ...
        socket.on("join_session", async (payload) => {
            ...
            if (user.role === "astrologer" && session.status === "waiting") {
                const upd = await pool.query(`UPDATE chat_sessions
           SET status = 'active', started_at = now()
           WHERE id = $1 AND status = 'waiting'
           RETURNING started_at`, [sessionId]);
            ...
            if (latest?.status === "active") {
                ...
                socket.emit("session_tick", {
                    elapsedMinutes: Math.max(0, Math.floor((Date.now() - startedAtMs) / 60_000)),
        ...
        socket.on("send_message", async (payload) => {
            ...
            const senderType = user.role === "astrologer" ? "astrologer" : "user";
            const ins = await pool.query(`INSERT INTO messages (session_id, sender_id, sender_type, content, is_automated)
         VALUES ($1, $2, $3::message_sender_type, $4, false)
         RETURNING id, created_at`, [sessionId, uid, senderType, content]);
            const row = ins.rows[0];
            if (!row) {
                return;
            }
            try {
                await pool.query(`INSERT INTO session_messages_archive (session_id, sender_role, content, created_at)
           VALUES ($1, $2, $3, NOW())`, [sessionId, senderType, content]);
            }
            catch (archiveErr) {
                console.error("message archive insert failed:", archiveErr);
            }
            const state = ensureLiveSession(sessionId, session.started_at ? new Date(session.started_at).getTime() : Date.now());
            ...
            const recipientUserId = uid === session.user_id ? session.astrologer_user_id : session.user_id;
            const senderName = uid === session.astrologer_user_id
        ...
        socket.on("initiate_call", async (payload) => {
            ...
            await pool.query(`UPDATE chat_sessions
         SET session_type = $1,
             call_channel_name = $2
         WHERE id = $3`, [callType, channelName, sessionId]);
        ...
        socket.on("accept_call", async (payload) => {
            ...
            const callType = (session.session_type ?? "voice");
            socket.emit("call_ready", {
                channelName,
                token,
                uid: accepterUid,
        ...
        socket.on("disconnect", () => {
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (role === "astrologer") {
                        const statusResult = await pool.query(`UPDATE astrologers
               SET is_online = false
               WHERE user_id = $1
               RETURNING id`, [uid]);
                    ...
                    const open = await pool.query(`SELECT cs.id
             FROM chat_sessions cs
             INNER JOIN astrologers a ON a.id = cs.astrologer_id
             WHERE cs.status IN ('waiting', 'active')
               AND (cs.user_id = $1 OR a.user_id = $1)`, [uid]);
